    ...
    var body: some View {
        VStack{
            VStack(alignment: .leading){
                EntryFieldView(textBinding: $viewModel.newPassword, placeholderString: "New Password", isSecureField: true, iconImagename: "lock.fill")
                    .onChange(of: viewModel.newPassword){ _,_ in
                        viewModel.verifyPasswordMatch()
                    }
                EntryFieldView(textBinding: $viewModel.confirmPassword, placeholderString: "Confirm Password", isSecureField: true, iconImagename: "exclamationmark.lock.fill")
